...
export const fetchFromIssuer = async (
...
const internalFetch = async (
  ...
  try {
    ...
    if (e.message === 'AbortError' || e.message === 'Network request failed') {
      if (numberRetry <= 1) {
...
export const performRevocationRequestAsync = async (
  url,
  token,
...
export const performTokenRequestAsync = async (
  url,
  details,
  extras,
  oldTokens,
...
export const performAuthorizationRequestAsync =
  (storage: any) => async (url, extras: StringMap) => {
...
export const performFirstTokenRequestAsync =
  (storage: any) =>
  async (url, extras, tokenRenewMode: string, timeoutMs = 10000) => {
    extras = extras ? { ...extras } : {};
    ...
    await Promise.all([
      storage.setCodeVerifierAsync(null),
      storage.setStateAsync(null),
    ]);
    if (response.status !== 200) {
      return { success: false, status: response.status };
    }
