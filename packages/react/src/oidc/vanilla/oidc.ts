...
const loginCallbackWithAutoTokensRenewAsync = async (
  oidc
...
export class Oidc {
  ...
  constructor(configuration: OidcConfiguration, configurationName = 'default') {
    ...
    if (configuration.silent_redirect_uri && !configuration.silent_login_uri) {
      silent_login_uri = `${configuration.silent_redirect_uri
        .replace('-callback', '')
        .replace('callback', '')}-login`;
    }
    this.configuration = {
      ...configuration,
      silent_login_uri,
      ...
      refresh_time_before_tokens_expiration_in_second:
        configuration.refresh_time_before_tokens_expiration_in_second ?? 60,
    ...
    this.initAsync(
      ...
      this.configuration.authority_configuration
    );
  }
  ...
  static get(name = 'default') {
    ...
    if (
      !Object.prototype.hasOwnProperty.call(oidcDatabase, name) &&
      isInsideBrowser
    ) {
...
Please checkout that you are using OIDC hook inside a <OidcProvider configurationName="${name}"></OidcProvider> compoment.`);
  ...
  _silentLoginCallbackFromIFrame() {
    if (
      this.configuration.silent_redirect_uri &&
      this.configuration.silent_login_uri
    ) {
      const queryParams = getParseQueryStringFromLocation(window.location.href);
      window.top.postMessage(
        `${this.configurationName}_oidc_tokens:${JSON.stringify({
          tokens: this.tokens,
  ...
  async initAsync(
    ...
    const localFuncAsync = async () => {
      ...
      const serviceWorker = await initWorkerAsync(
        this.configuration.service_worker_relative_url,
        this.configurationName
      );
      const storage = serviceWorker ? window.localStorage : null;
      return await fetchFromIssuer(
        authority,
        tenant_id,
  ...
  async tryKeepExistingSessionAsync(): Promise<boolean> {
    ...
    const funcAsync = async () => {
      ...
      try {
        ...
        const oidcServerConfiguration = await this.initAsync(
          configuration.authority,
          configuration.tenant_id,
          configuration.authority_configuration
        );
        serviceWorker = await initWorkerAsync(
          configuration.service_worker_relative_url,
          this.configurationName
        );
        if (serviceWorker) {
          ...
          if (tokens) {
            ...
            // @ts-ignore
            this.timeoutId = autoRenewTokens(
              this,
            ...
            // @ts-ignore
            await this.startCheckSessionAsync(
          ...
          if (configuration.service_worker_relative_url) {
            this.publishEvent(
              eventNames.service_worker_not_supported_by_browser,
              {
  ...
  async loginCallbackAsync(isSilenSignin = false) {
    ...
    const loginCallbackLocalAsync =
      async (): Promise<InternalLoginCallback> => {
  ...
  async synchroniseTokensAsync(
    ...
    while (!navigator.onLine && document.hidden) {
      ...
      this.publishEvent(eventNames.refreshTokensAsync, {
    ...
    while (document.hidden && numberTryHidden > 0) {
      ...
      numberTryHidden--;
      this.publishEvent(eventNames.refreshTokensAsync, {
        message: `wait because navigator is hidden try ${numberTryHidden}`,
      });
    }
    ...
    const localsilentLoginAsync = async () => {
      try {
        ...
        if (
          exceptionSilent &&
          exceptionSilent.message &&
          exceptionSilent.message.startsWith('oidc')
        ) {
          updateTokens(null);
      ...
      this.publishEvent(eventNames.refreshTokensAsync_error, {
        message: 'refresh token silent return',
      });
      return await this.synchroniseTokensAsync(
        null,
        nextIndex,
        forceRefresh,
        extras,
    ...
    try {
      const { status, tokens, nonce } = await this.syncTokensInfoAsync(
        configuration,
        this.configurationName,
        this.tokens,
        forceRefresh
      ...
      switch (status) {
        ...
        default: {
          ...
          const localFunctionAsync = async () => {
            ...
            const tokenResponse = await performTokenRequestAsync(
      ...
      return this.synchroniseTokensAsync(
        refreshToken,
        nextIndex,
        forceRefresh,
        extras,
  ...
  async syncTokensInfoAsync(
    ...
    if (serviceWorker) {
      ...
      if (status === 'LOGGED_OUT') {
        return {
          tokens: null,
          status: 'LOGOUT_FROM_ANOTHER_TAB',
          nonce: nullNonce,
        };
  ...
  loginCallbackWithAutoTokensRenewAsync(): Promise<LoginCallback> {
    ...
    this.loginCallbackWithAutoTokensRenewPromise =
      loginCallbackWithAutoTokensRenewAsync(this);
  ...
  async renewTokensAsync(extras: StringMap = null) {
    ...
    // @ts-ignore
    this.renewTokensPromise = renewTokensAndStartTimerAsync(
      this,
      this.tokens.refreshToken,
      true,
      extras
  ...
  async logoutAsync(
    ...
    this.logoutPromise = logoutAsync(this, oidcDatabase)(
      callbackPathOrUrl,
      extras
    );
